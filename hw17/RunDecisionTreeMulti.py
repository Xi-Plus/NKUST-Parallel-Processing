...
def extract_features(record, featureEnd):
    numericalFeatures = [convert_float(field) for field in record[:-1]]
...
def PrepareData(sc):
    # ----------------------1.匯入並轉換資料-------------
    print("開始匯入資料...")
    rawData = sc.textFile(Path + "Grisoni_et_al_2016_EnvInt88.csv")
...
def PredictData(sc, model):
    ...
    for lp in labelpointRDD.take(100):
        ...
        if label == predict:
            ...
            wrong += 1
        print("Data:" + str(features)
              + "....==>預測:" + str(predict)
              + " 實際:" + str(label) + "結果:" + result)
    print('正確 %d 筆，錯誤 %d 筆' % (correct, wrong))
...
def evalParameter(trainData, validationData, evaparm, impurityList, maxDepthList, maxBinsList):
    ...
    showchart(df, evaparm, 'accuracy', 'duration', 0.3, 1.0)
